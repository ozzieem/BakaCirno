...
func test_button_as_music():
	...
	if button_play_select and music_player:
		...
		await get_tree().process_frame
		print("   Button sound as music - playing: ", music_player.playing)
		print("   If you hear this, the music player works but CirnoThemeSong.wav has issues")
	else:
		print("❌ Cannot test - button_play_select or music_player is null")

# Detailed WAV file analysis
func analyze_music_file():
	print("🔍 ANALYZING MUSIC FILE: CirnoThemeSong.wav")
	if playing_song and playing_song is AudioStreamWAV:
		var wav_stream = playing_song as AudioStreamWAV
		print("   WAV Properties:")
		print("   - Duration: ", wav_stream.get_length(), " seconds")
		print("   - Loop mode: ", wav_stream.loop_mode)
		print("   - Loop begin: ", wav_stream.loop_begin)
		print("   - Loop end: ", wav_stream.loop_end)
		print("   - Mix rate: ", wav_stream.mix_rate)
		print("   - Stereo: ", wav_stream.stereo)
		print("   - Data size: ", wav_stream.data.size() if wav_stream.data else "No data")

		# Check if the WAV data is valid
		if wav_stream.data and wav_stream.data.size() > 0:
			print("   ✅ WAV data is present (", wav_stream.data.size(), " bytes)")
		else:
			print("   ❌ WAV data is missing or empty!")

		# Try to analyze the format
		if wav_stream.format != AudioStreamWAV.FORMAT_8_BITS and wav_stream.format != AudioStreamWAV.FORMAT_16_BITS:
			print("   ⚠️ Unusual format detected: ", wav_stream.format)
		else:
			print("   ✅ Standard format: ", wav_stream.format)
	else:
		print("   ❌ playing_song is not AudioStreamWAV or is null")
